...
- 将 Token 面板标题从 `TOKENS / PC SYNC` 简化为 `TOKENS / SYNC`，与 HTML 原型保持一致。

- Token 总量、各模型用量及图表峰值自动从 M 进位到 B；B 保留两位小数，四舍五入规则与 HTML 原型一致。

- 修复 CW2017 已在 I²C 总线上应答、Header 电量却始终显示 `--%` 的问题：加载并校验厂家为 520mAh 电芯提供的 80 字节 profile，执行规定的睡眠/重启时序，并在向 UI 提供电量计前最多等待 5 秒取得有效 SOC。启动时新增首个有效 SOC 与电芯电压日志，用于硬件验证。
- 新增右对齐的蓝牙与电量状态组：蓝牙灰色表示 Mac 桥接端未连接，信号绿色表示已连接；蓝牙与电量间距为 4 像素，电量由自适应宽度的百分比和仿 iPhone 轮廓图标组成。电量填充按比例变化，40% 及以下变黄，20% 及以下变红。收紧 Header 与分隔线的间距，并将仪表盘整体上移 5 像素，让纵向节奏更紧凑。Codex 额度改为独立于已不使用的 Claude statusline 额度缓存发送；同时按 10080 分钟窗口识别 Codex 七天额度，因为当前 Pro 返回可能将其放在 `primary`，旧格式则放在 `secondary`。底部仅显示一条带精确百分比的 `CODEX` 剩余额度进度条，并将标签与进度条的间距增加 6 像素。
- 将豆包语音主屏重构为 240 x 320 的「AI PASSPORT」Type AA 数据控制台：空闲态可以在 Codex/Claude 最近一天的分时用量和最近七天的每日用量之间切换，总量、图表与前三模型会同步变化。双击确定键切换范围；单击仍然发送，但需等待按键组件约 180 ms 的双击判定窗口。录音期间则暂时用计时器与实时输入电平替换用量主卡。macOS 桥接端只从本地时间戳、模型标识与 Token 计数生成数据，不发送任何对话内容；数据每五分钟在非录音状态刷新，并分别发送带校验和的日、周 BLE 数据包，使旧版仅支持单日数据的固件继续兼容。主机端测试覆盖两种数据包与图表缩放。
- 恢复 PC 桥接端的 BLE 名称与特征 UUID 常量：一次重构删掉了定义，却仍让扫描与订阅路径引用它们，导致 `recv-ble` 以 `NameError` 退出；现在可以正常进入设备发现。豆包按住说话的快捷键改为通过 Quartz 发送 macOS 原生右 Command 键码，因为 PyAutoGUI 的通用 Command 实际映射到左侧；准确率工具与安装说明同步使用同一快捷键。
- 关闭 NimBLE 每条通知的 INFO 日志。它对每个 GATT notify 都打两行，而 `esp_log` 同步写 USB-serial 控制台，音频每 15 ms 一帧、一帧一个 notify，控制台写入直接拖住采集任务，把实际送达率压在 81-88%。警告和错误仍会打印，格式字符串也从 Flash 中消失。
- 不再把 `voice_ble_ready()` 的瞬时 false 当作录音结束。该函数只反映连接句柄，控制器繁忙时可能瞬时读到 false，一次抖动就把状态打回空闲，于是录音界面（计时、电平、状态字）始终看不到，尽管音频一直在推。现在只有用户按停才停止；真正断链仍由重连路径处理。
- 录音屏新增实时电平表，用现有标签里的 ASCII 文本实现，而不是新增控件：此前基于格子的电平表新增了 12 个 `lv_obj`，把 LVGL 任务拖过任务看门狗超时，并因 `on_key` 要等 LVGL 锁而导致所有按键失效。RMS 每 4 帧算一次、每 4 个采样取 1 个——逐帧遍历全部采样会让送达率掉 4 个点，因为它位于采集读取与 notify 之间的工作循环里。它是设备上唯一能证明麦克风真的在收音的显示；此前链路断掉和正常工作看起来完全一样。
- 修复屏幕文字显示方块。`·`（U+00B7）与 `✕`（U+2715）落在子集 CJK 字库已声明的 cmap 范围内，却没有对应字形，因此渲染为占位方块。该字库以 256 码位为一块声明了 87 段范围、实际仅 21412 个字形，所以"码位落在已声明范围内"不能作为字形存在的判据，必须解析 cmap 条目。
- 录音与删除键改为按下即触发，而非单击。`BSP_BTN_CLICK` 对应按键组件的 `BUTTON_SINGLE_CLICK`，必须等手指抬起并度过约 180 ms 的双击判定窗口才会发出，导致录音在按下之后很久才开始，手感迟钝。两个动作都可恢复，等待第二次点击没有收益。发送保留单击，因为确定键的长按必须仍可区分——长按用于离开当前屏幕。
- 上键长按清空整行（控制码 5）。PC 侧发送全选再删除，而不是固定次数的退格：它无法得知输入法已提交多少文字，固定次数要么留下残留、要么删掉前面的内容。
- 在 Claude 额度旁显示 Codex 额度。Codex 没有 statusline 钩子，但其 CLI 会把服务端返回的限额原样记录在每个会话 rollout 中，因此 agent 从最新的一个里读取 7 天窗口数据。额度数据包新增一个字节承载它；设备仍接受旧的 7 字节格式。
- 移除屏幕上的按键图例与操作说明文字。三个按键学一次就会，之后这些文字只是噪声。
- 修复虚拟麦克风只送出 68% 实时音频，导致流式转写（豆包输入法）每句都被切断：BLE 每个连接事件大约只放行一个 notify，所以**帧大小**才是链路带宽上限，而非采样率。macOS 实际稳定在约 15 ms 间隔，10 ms 的帧因此永久欠三分之一，这些空洞被识别成句尾。现改为每帧 240 采样（15 ms、480 B —— ATT MTU 512 内最大的整数 PCM 帧），实测 101% 实时率。`voice_ble_send_audio` 保持非阻塞（阻塞会顺带卡住采集，一次停顿丢好几帧）；worker 保留一帧未发出的音频，下一轮先补发再采新帧，使两个 notify 能排入同一个连接事件，链路卡顿后能追回而非永久丢音。
- 把按句播放改为一条常开的虚拟麦克风流：流式 ASR 容忍延迟但不容忍空洞，所以输出流只打开一次、永不停止，BLE 帧进入弹性队列（200 ms 预缓冲、800 ms 上限），欠载时保持上一采样而不是塞静音。豆包录音键在整个会话期间保持按下，队列排空后才松开，最后一句不会被截断；keyDown/keyUp 严格配对，并用会话计数让仍在等待的排空线程直接退出——重复的 keyDown 会让按键卡在按下状态，导致下一次按键无法停止豆包。
