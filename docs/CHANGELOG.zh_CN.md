...
- 在仪表盘标题栏明确展示 BLE 状态：现在会区分「连接中」「已连接」、曾连接后发生的「连接丢失」以及初始化错误，不再只依赖没有文字说明的颜色点。修复 Claude statusline 额度缓存不存在时整个额度栏为空的问题：Codex 额度改为独立发送；同时按 10080 分钟窗口识别 Codex 七天额度，因为当前 Pro 返回可能将其放在 `primary`，旧格式则放在 `secondary`。底部标签也由 `C` / `X` 改为完整的 `CLAUDE` / `CODEX`。
- 将豆包语音主屏重构为 240 x 320 的「AI PASSPORT」Type AA 数据控制台：空闲态展示本地 Codex/Claude 最近 24 小时 Token 总量、24 根分时柱、用量最高的三个模型、连接/电池状态及两项周额度；录音期间则暂时用计时器与实时输入电平替换用量主卡。macOS 桥接端只从本地时间戳、模型标识与 Token 计数生成数据，不发送任何对话内容；数据每五分钟在非录音状态刷新，并通过带校验和的定长 BLE 数据包传输，配套主机端解析与缩放测试。
