...
// 厂家为本机优特利 520mAh 电芯提供的参数。CW2017 没有可用于这颗电芯的
// 通用内置 profile；未加载时芯片虽然能响应 VERSION，但 SOC 会长期无效。
...
static int cw_wait_soc_ready(void) {
    ...
    ESP_LOGE(TAG, "等待首次 SOC 计算超时:last_soc=0x%02X", soc);
    return -1;
}
...
esp_err_t bsp_battery_init(void) {
    ...
    ESP_LOGI(TAG, "SOC 就绪:%d%% voltage=%dmV",
